...
class CurrentWeatherActivity : AppCompatActivity() {
    ...
    private fun populateUI() {
        ...
        current_weather_provider.text = SharedPreferenceUtil.getInstance(applicationContext).getSavedWeatherProvider()
        if (currentWeather != null) {
            current_weather_city_country_label.text = currentWeather.location
            current_weather_temp_value.text = getString(R.string.current_temp_label, currentWeather.currentTemp.toString(), UnitsUtil.getFahrenheitUnit())
            if (currentWeather.windSpeed != null) {
                current_weather_wind.visibility = View.VISIBLE
                current_weather_wind.text = getString(R.string.current_weather_wind_label, currentWeather.windSpeed.toString(), UnitsUtil.getWindSpeedUnits())
            }
            if (currentWeather.minTemp != null) {
                current_weather_min_temp.visibility = View.VISIBLE
                current_weather_min_temp.text = getString(R.string.current_weather_min_label, currentWeather.minTemp.toString(), UnitsUtil.getFahrenheitUnit())
            }
            if (currentWeather.maxTemp != null) {
                current_weather_max_temp.visibility = View.VISIBLE
                current_weather_max_temp.text = getString(R.string.current_weather_max_label, currentWeather.maxTemp.toString(), UnitsUtil.getFahrenheitUnit())
            }
        }
    }
}
